...
<p align="center">
  <img src="assets/demo.svg" alt="claude usage demo" width="620"/>
</p>

<p align="center">
  <a href="https://omsatyaswaroop29.github.io/claude-usage-contribution/demo/"><strong>Try the full interactive demo</strong></a> · <a href="docs/ROADMAP.md">Roadmap</a>
</p>
...
<details>
<summary><code>claude usage --rate-limit</code> — are you about to hit a cap?</summary>
...
<details>
<summary><code>claude usage --mcp-breakdown</code> — where is your context window going?</summary>
...
<details>
<summary><code>claude usage --watch</code> — real-time monitoring</summary>
...
<details>
<summary><code>claude usage --json</code> — pipe it anywhere</summary>
...
Formatting utilities
  ✓ formatTokenCount adds thousands separators
  ✓ formatCost formats to 2 decimal places with $
  ✓ formatPct rounds to whole number
  ✓ barChart produces correct visual

RateLimitCollector
  ✓ should parse standard rate-limit headers
  ✓ should prefer unified-status when available
  ✓ should handle missing headers gracefully
  ✓ should handle malformed unified-status JSON
  ✓ should produce valid statusLine JSON
  ✓ isFresh returns false before any update
  ✓ isFresh returns true after update

MCPIntrospector
  ✓ should estimate tokens from character count
  ✓ should round up partial tokens
  ✓ should estimate tool cost from name + description + schema
  ✓ should analyze a server with multiple tools
  ✓ should analyze multiple servers and compute total budget
  ✓ should produce valid statusLine extension

TerminalFormatter
  ✓ should format a summary without crashing
  ✓ should format JSON output as valid JSON
...
UsageCommand integration
  ✓ should execute summary with empty data without crashing
  ✓ should execute --json and return valid JSON
  ✓ should execute --rate-limit without crashing
  ✓ should execute --mcp-breakdown without crashing
...
├── assets/
│   └── demo.svg                    ← Animated terminal showcase
├── demo/
│   └── index.html                  ← Full interactive demo with pixel mascot
├── docs/
